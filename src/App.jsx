...
const SEED_VOCAB=[
  ...
  {id:"familia",basque:"Familia",english:"Family",cefr:"A1",topic:"family",pronunciation:"fah-MEE-lee-ah",notes:"Familia comes from Latin/Spanish. The more traditional Basque word for family is etxea (house/home) - reflecting how family centerd on the household.",example:{basque:"Familia handia dut.",english:"I have a big family."}},
  ...
  {id:"bertsoa",basque:"Bertsoa",english:"Improvised verse",cefr:"A2",topic:"culture",pronunciation:"ber-TSOH-ah",notes:"Improvised sung verse in strict meter and rhyme. Bertsolaritza is the art of improvising these verses - competitions (txapelketak) fill sports arenas. A living oral tradition unique to Basque culture.",example:{basque:"Bertsoa ederra da.",english:"The verse is beautiful."}},
  ...
  {id:"bertsolaria",basque:"Bertsolaria",english:"Verse singer",cefr:"B1",topic:"culture",pronunciation:"ber-tsoh-LAH-ree-ah",notes:"Bertsolaria = verse singer (bertsolaritza practitioner). Improvises in strict meter and rhyme on given themes. National championships fill the BEC arena in Bilbo (Bilbao) with 15,000 spectators. A uniquely Basque art form.",example:{basque:"Bertsolaria bikaina da.",english:"The verse singer is excellent."}},
  ...
  {id:"zuri_horia",basque:"Zuri-horia",english:"Cream / Off-white",cefr:"A2",topic:"colors",pronunciation:"ZOO-ree HOR-ee-ah",notes:"Cream or off-white. Compound colors in Basque are formed by combining two color words. Zuri = white + horia = yellow = cream. Very common for describing walls and fabrics.",example:{basque:"Pareta zuri-horia da.",english:"The wall is cream colored."}},
  ...
  {id:"auzoa",basque:"Auzoa",english:"Neighborhood / Village",cefr:"A1",topic:"society",pronunciation:"ow-SOH-ah",notes:"Auzoa = neighborhood or small village. Auzo = neighborhood (root). Auzokoa = neighbor. Auzolan = community work (neighborhood work) - the Basque tradition of collective voluntary labor. A beautiful concept: neighbors working together for the common good.",example:{basque:"Nire auzoa txikia da.",english:"My neighborhood is small."}},
      ...
      <div style={{padding:"12px 14px 14px"}}>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}>
          <div style={{display:"flex",gap:4,flex:1}}>
            {[10,20,30].map(function(n){var tooMany=n>poolSize;var validOpts=[10,20,30].filter(function(x){return x<=poolSize;});var effSel=sessionLen<=poolSize?sessionLen:(validOpts.length?validOpts[validOpts.length-1]:10);var active=effSel===n;var nn=n;return(
              <button key={n} disabled={tooMany} onClick={function(){if(!tooMany)setSessionLen(nn);}} style={{flex:1,padding:"7px 4px",borderRadius:10,border:"1.5px solid "+(active?L.dark:"#E8E8E8"),backgroundColor:active?L.color:"#F2F2F7",color:active?"#fff":"#8E8E93",fontSize:12,fontWeight:active?800:500,cursor:tooMany?"not-allowed":"pointer",fontFamily:"inherit",transition:"all 0.12s",opacity:tooMany?0.35:1}}>
      ...
      <div style={{display:"flex",gap:10,marginTop:16}}>
        {isPro&&(score.genuinelyWrong>0||score.closeButWrong>0)?(
          <button style={{flex:2,backgroundColor:"#19A85A",color:"#fff",border:"none",borderRadius:14,padding:"14px",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 0 #0E7A40"}} onClick={function(){onRetry(score.missedWords.concat(score.closeWords).map(function(w){return w.id;}).filter(function(v,i,a){return a.indexOf(v)===i;}));}}>
            Practice mistakes ({score.missedWords.length+score.closeWords.length})
          </button>
        ):(!isPro&&(score.genuinelyWrong>0||score.closeButWrong>0))?(
        ...
        <button style={{flex:2,backgroundColor:"#19A85A",color:"#fff",border:"none",borderRadius:16,padding:"14px",fontSize:16,fontWeight:900,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 0 #0E7A40",letterSpacing:-0.3}} onClick={function(){if(isLesson){onBackToLessons&&onBackToLessons();}else{onRetry([]);}}}>{isLesson?"Back to lessons \u2192":"New quiz \u2192"}</button>
      </div>
    </div>
  </div>);}
