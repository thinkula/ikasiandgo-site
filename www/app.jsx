...
function App(){
  ...
  useEffect(function(){async function load(){try{var rs=await Promise.all([window.storage.get("streak_data"),window.storage.get("srs_data")]);if(rs[0]&&rs[0].value){var d=JSON.parse(rs[0].value),today=new Date().toDateString(),yest=new Date(Date.now()-86400000).toDateString();setStreak((d.lastDay===today||d.lastDay===yest)?(d.streak||0):0);setLongest(d.longest||0);setTotalSess(d.totalSessions||0);}if(rs[1]&&rs[1].value){try{var parsed=JSON.parse(rs[1].value);if(typeof parsed==="object"&&parsed!==null)setSrsData(parsed);}catch(e){console.warn("SRS data corrupted, resetting");}}
        ...
        (async function(){
          try{
            ...
            if(res.ok){var vj=await res.json();if(vj&&vj.vocabulary){_VOCAB=vj.vocabulary;_WC=getWC();setVocabVersion(function(v){return v+1;});window.storage.set("vocab_cache",JSON.stringify(vj)).catch(function(){});}}
          }catch(e){}
        })();
      }load();},[]);
  ...
  setSrsData(function(){return nd;});try{await window.storage.set("srs_data",JSON.stringify(nd));}catch(e){}return nd;}
  ...
  try{
    ...
    await window.storage.set("session_history",JSON.stringify(hist));
  }catch(e){}
}
async function recordSession(){var today=new Date().toDateString(),yest=new Date(Date.now()-86400000).toDateString(),ns=streak;try{var r=await window.storage.get("streak_data");var d=r&&r.value?JSON.parse(r.value):{};if(d.lastDay===today)ns=d.streak||streak;else ns=(d.lastDay===yest?(d.streak||0):0)+1;var nl=Math.max(ns,d.longest||0);await window.storage.set("streak_data",JSON.stringify({streak:ns,longest:nl,lastDay:today,totalSessions:(d.totalSessions||0)+1}));setStreak(ns);setLongest(nl);setTotalSess((totalSess||0)+1);}catch(e){setStreak(streak+1);}return ns;}
  ...
  async function finishQuiz(res){var sc=scoreSession(res);var worthStreak=res.length>=5;await Promise.all([worthStreak?recordSession():Promise.resolve(),updateSRS(res),saveSessionHistory(sc.accuracy,cfg?cfg.cefr:"A1",cfg?cfg.topic:"all")]);setResults(res);setScreen("results");}
  return(<div style={{fontFamily:"Nunito,system-ui,-apple-system,sans-serif",backgroundColor:"#F6F6F6",minHeight:"100vh"}}>
    {screen==="home"&&<HomeScreen onStart={startQuiz} isPro={isProOrTrial} isTrialActive={isTrialActive} trialUntil={trialUntil} streak={streak} streakLoaded={streakLoaded} srsStats={srsStats} srsLoaded={srsLoaded} onBrowse={function(){setScreen("browse");}} onUpgrade={function(){setScreen("paywall");}} onReplayIntro={function(){setShowOnb(true);}} totalSessions={totalSess} srsData={srsData}/>}
    ...
    {screen==="results"&&<ResultsScreen results={results} streak={streak} longest={longest} totalSessions={totalSess} srsStats={srsStats} isPro={isProOrTrial} sessionHistory={sessionHistory} onRetry={function(ids){if(cfg)startQuiz(cfg,ids||[]);else setScreen("home");}} onHome={function(){setScreen("home");}} onUpgrade={function(){setScreen("paywall");}}/>}
    {screen==="paywall"&&<PaywallScreen
    trialAvailable={!trialUntil&&!isPro}
    trialDays={3}
    onTrial={function(){var until=new Date(Date.now()+3*86400000);setTrialUntil(until);window.storage.set("trial_until",until.toISOString()).catch(function(){});setScreen("home");}}
...
function HomeScreen(props){
  var VOCABULARY=_VOCAB;
  var WC=_WC;
  var onStart=props.onStart,isPro=props.isPro,streak=props.streak,streakLoaded=props.streakLoaded,srsStats=props.srsStats,srsLoaded=props.srsLoaded,onBrowse=props.onBrowse,onUpgrade=props.onUpgrade,onReplayIntro=props.onReplayIntro;var totalSessions=props.totalSessions||0;var srsData=props.srsData||{};var isTrialActive=props.isTrialActive;var trialUntil=props.trialUntil;
  ...
  var srsCard=null;
  if(srsLoaded&&(srsStats.mastered>0||srsStats.due>0||srsStats.learning>0)){
  ...
  var levelCards=["A1","A2","B1","B2"].map(function(key){var L2=CL[key];var active=lvl===key;var k=key;
    return React.createElement("button",{key:key,onClick:function(){if(!isPro&&k!=="A1"){onUpgrade();return;}setLvl(k);},style:{borderRadius:16,padding:"14px 12px",display:"flex",flexDirection:"column",gap:5,cursor:"pointer",border:"2px solid "+(active?L2.dark:"#E8E8E8"),backgroundColor:active?L2.color:"#fff",boxShadow:active?"0 6px 0 "+L2.dark:"0 2px 8px rgba(0,0,0,0.06)",transform:active?"translateY(-2px)":"none",textAlign:"left",fontFamily:"inherit",transition:"all 0.18s ease"}},
      React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}},
        React.createElement("span",{style:{fontSize:10,fontWeight:800,color:active?"rgba(255,255,255,0.9)":L2.color,backgroundColor:active?"rgba(255,255,255,0.25)":L2.bg,padding:"2px 8px",borderRadius:12,border:"1px solid "+(active?"rgba(255,255,255,0.3)":L2.color+"55")}},key),
  ...
  React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}},
    ...
    React.createElement("p",{style:{margin:0,fontSize:10,color:active?"rgba(255,255,255,0.6)":"#BBB",fontWeight:700}},(masteredByLevel[key]>0?masteredByLevel[key]+" mastered":""))
  ),
  ...
  return(<div style={{maxWidth:420,margin:"0 auto",display:"flex",flexDirection:"column",minHeight:"100vh"}}>
    <div style={{background:"linear-gradient(155deg,#0E7A40 0%,#19A85A 100%)",paddingTop:56,paddingLeft:16,paddingRight:16,paddingBottom:0,flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:12,paddingBottom:16}}>
        <Logo size={32}/>
        <div style={{flex:1}}>
          <h1 style={{margin:0,fontSize:20,fontWeight:900,color:"#fff",letterSpacing:-0.5}}>Ikasi & Go™</h1>
          <p style={{margin:"1px 0 0",fontSize:11,color:"rgba(255,255,255,0.65)",fontWeight:500}}>Basque vocabulary trainer</p>
        </div>
        {isTrialActive&&(
    ...
    <div style={{padding:"8px 16px calc(72px + env(safe-area-inset-bottom, 0px))",display:"flex",flexDirection:"column",gap:16,flex:1,overflowY:"auto"}}>
      ...
      <section style={{backgroundColor:"#fff",borderRadius:20,padding:"16px",border:"1px solid #F0F0F0",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
    ...
    {modal&&(function(){var ml=CL[modal];return(
      <div style={{position:"fixed",inset:0,backgroundColor:"rgba(0,0,0,0.55)",zIndex:200,display:"flex",alignItems:"flex-end"}} onClick={function(){setModal(null);}}>
        <div style={{backgroundColor:"#fff",borderTopLeftRadius:28,borderTopRightRadius:28,width:"100%",maxWidth:420,margin:"0 auto",overflow:"hidden"}} onClick={function(e){e.stopPropagation();}}>
          <div style={{backgroundColor:ml.color,padding:"20px 20px 16px"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <span style={{fontSize:28,fontWeight:900,color:"rgba(255,255,255,0.9)"}}>{modal}</span>
              <div style={{flex:1}}>
                <p style={{margin:0,fontSize:18,fontWeight:900,color:"#fff"}}>{ml.title}</p>
                <p style={{margin:"2px 0 0",fontSize:12,color:"rgba(255,255,255,0.8)"}}>{ml.tagline}</p>
              </div>
              <button style={{background:"rgba(255,255,255,0.2)",border:"none",color:"#fff",width:30,height:30,borderRadius:"50%",cursor:"pointer",fontSize:14,fontFamily:"inherit"}} onClick={function(){setModal(null);}}>✕</button>
            </div>
          </div>
          <div style={{padding:"16px 20px 32px",maxHeight:"70vh",overflowY:"auto"}}>
            <p style={{margin:"0 0 10px",fontSize:11,fontWeight:800,color:"#888",textTransform:"uppercase",letterSpacing:0.8}}>At this level you can...</p>
            {ml.canDo.map(function(c,i){return(
              <div key={i} style={{display:"flex",gap:10,marginBottom:8}}>
                <span style={{fontSize:12,fontWeight:800,color:"#fff",backgroundColor:ml.color,width:18,height:18,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>✓</span>
                <p style={{margin:0,fontSize:13,color:"#1A1A1A",fontWeight:500,lineHeight:1.4}}>{c}</p>
              </div>
            );})}
            <div style={{backgroundColor:ml.bg,borderRadius:12,padding:"12px 14px",marginTop:12,border:"1.5px solid "+ml.color+"55"}}>
              <p style={{margin:0,fontSize:13,color:"#555",lineHeight:1.55,fontWeight:500}}>Tip: {ml.tip}</p>
            </div>
            <div style={{display:"flex",gap:8,marginTop:12}}>
              <div style={{flex:1,backgroundColor:"#F6F6F6",borderRadius:10,padding:"10px",textAlign:"center"}}>
                <p style={{margin:0,fontSize:16,fontWeight:900,color:ml.color}}>{WC[modal]}</p>
                <p style={{margin:0,fontSize:10,color:"#888",fontWeight:600}}>WORDS</p>
              </div>
              <div style={{flex:1,backgroundColor:"#F6F6F6",borderRadius:10,padding:"10px",textAlign:"center"}}>
                <p style={{margin:0,fontSize:12,fontWeight:800,color:"#1A1A1A"}}>{ml.studyHours}</p>
                <p style={{margin:0,fontSize:10,color:"#888",fontWeight:600}}>GUIDED STUDY</p>
              </div>
            </div>
            <button style={{width:"100%",marginTop:14,border:"none",borderRadius:14,padding:"14px",fontSize:15,fontWeight:800,color:"#fff",cursor:"pointer",backgroundColor:ml.color,boxShadow:"0 4px 0 "+ml.dark,fontFamily:"inherit"}} onClick={function(){setLvl(modal);setModal(null);}}>Study {modal} words</button>
          </div>
        </div>
      </div>
    );})()}
...
function QuizScreen(props){
  ...
  function submit(ans){if(result)return;var qForCheck=isSnoozed&&origType==="typing"?Object.assign({},q,{correct:snoozeCorrect,mode:"multipleChoice"}):q;var r=checkAnswer(qForCheck,ans);setResult(r);if(!r.correct)setWrongCount(function(n){return n+1;});var newLatest=latest.current.concat([Object.assign({},r,{question:q})]);latest.current=newLatest;if(newLatest.length>0&&newLatest.length%5===0&&props.onAutoSave)props.onAutoSave(newLatest);}
  var advancing=React.useRef(false);function advance(){if(advancing.current)return;advancing.current=true;if(idx+1>=questions.length){onFinish(latest.current);}else{setIdx(function(i){return i+1;});setSel(null);setTyped("");setResult(null);setShowNote(false);setWrongCount(0);advancing.current=false;}}
  ...
  var snoozeOpts=useMemo(function(){
    ...
    var sl2=dp2.filter(function(w){return w.cefr===q.word.cefr;});
    var dist2=shuffled(sl2.length>=3?sl2:dp2).slice(0,3);
  ...
  return(<div style={{maxWidth:420,margin:"0 auto",display:"flex",flexDirection:"column",minHeight:"100vh",backgroundColor:"#F6F6F6",animation:"fadeIn 0.2s ease"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,paddingTop:52,paddingLeft:16,paddingRight:16,paddingBottom:10,backgroundColor:"#fff",borderBottom:"1px solid #F0F0F0",flexShrink:0}}>
      ...
      <span style={{fontSize:10,fontWeight:800,color:lc,backgroundColor:lc+"18",padding:"3px 8px",borderRadius:20,flexShrink:0}}>{questions[0].word.cefr}</span>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"12px 16px calc(160px + env(safe-area-inset-bottom, 0px))",display:"flex",flexDirection:"column",gap:12}}>
      <div key={"card-"+idx} style={{backgroundColor:result?(result.correct?"#F0FBF4":result.wasClose?"#FFFBF0":"#FFF5F5"):"#fff",borderRadius:22,padding:"20px",border:"2px solid "+resColor,boxShadow:"0 4px 0 "+(result?(result.correct?"#0E7A40":result.wasClose?"#D97706":"#D98080"):"#E8E8E8"),transition:"all 0.25s",animation:"slideUp 0.2s ease"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <span style={{fontSize:11,fontWeight:800,padding:"4px 12px",borderRadius:20,color:isFB?"#7C3AED":isMC?"#D97706":"#19A85A",backgroundColor:isFB?"#F3EEFF":isMC?"#FFF8F0":"#EDFAF3",letterSpacing:0.2}}>{isFB?"Complete sentence":isMC?"Choose meaning":"Type in Basque"}</span>
        ...
        {(isMC||isType||result)&&!isFB&&(
          <div style={{textAlign:"center",marginBottom:12,marginTop:4}}>
            <h2 style={{margin:0,fontSize:isMC?(q.word.basque.length>14?20:q.word.basque.length>10?24:28):22,fontWeight:900,color:isMC?"#1A1A1A":"#0E7A40",letterSpacing:-0.5,wordBreak:"break-word",overflowWrap:"break-word"}}>{isMC?q.word.basque:q.prompt}</h2>
          ...
          <div>
            ...
            <div style={{borderRadius:14,overflow:"hidden",border:"2px solid "+(result.correct?"#6EE7B7":result.wasClose?"#FCD34D":"#FFBBBB")}}>
              ...
              <div style={{padding:"10px 14px",backgroundColor:"#FAFAFA",borderTop:"1px solid "+(result.correct?"#D4F5E9":result.wasClose?"#FDE68A":"#FFD5D5")}}>
                ...
                {isFB&&q.word.example&&<p style={{margin:"6px 0 0",fontSize:12,color:"#888",fontStyle:"italic"}}>{q.word.example.english}</p>}
      ...
      {isAnyMC&&!result&&opts.map(function(opt,i){var s=sel===opt;return(
        <button key={i} style={{display:"flex",alignItems:"center",gap:12,border:"2px solid "+(s?"#19A85A":"#E8E8E8"),borderRadius:16,padding:"15px 16px",cursor:"pointer",backgroundColor:s?"#F0FBF4":"#fff",width:"100%",textAlign:"left",fontFamily:"inherit",boxShadow:s?"0 3px 0 #0E7A40":"0 3px 0 #E0E0E0",transition:"all 0.12s",transform:s?"translateY(-1px)":"none"}} onClick={function(){if(!result){setSel(opt);submit(opt);}}}>
...
function ResultsScreen(props){
  ...
  var heroGrad=acc>=80?"linear-gradient(135deg,#0E7A40,#19A85A)":acc>=50?"linear-gradient(135deg,#0891B2,#38BDF8)":"linear-gradient(135deg,#C85070,#FF8C94)";
  ...
  return(<div style={{maxWidth:420,margin:"0 auto",backgroundColor:"#F6F6F6",minHeight:"100vh",animation:"fadeIn 0.2s ease"}}>
    {acc===100&&<Confetti/>}
    <div style={{background:heroGrad,padding:"28px 20px 24px",display:"flex",flexDirection:"column",alignItems:"center",borderBottomLeftRadius:32,borderBottomRightRadius:32}}>
      ...
      <p style={{margin:"0 0 20px",fontSize:14,color:"rgba(255,255,255,0.85)",fontWeight:600}}>{score.correct}/{score.total} correct - {score.grade.sub}</p>
      ...
      <div style={{display:"flex",gap:10,marginTop:16}}>
        {isPro&&(score.genuinelyWrong>0||score.closeButWrong>0)?(
          <button style={{flex:2,backgroundColor:"#19A85A",color:"#fff",border:"none",borderRadius:14,padding:"14px",fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 0 #0E7A40"}} onClick={function(){onRetry(score.missedWords.concat(score.closeWords).map(function(w){return w.id;}).filter(function(v,i,a){return a.indexOf(v)===i;}));}}>
            Retry ({score.missedWords.length+score.closeWords.length})
          </button>
        ):(
        ...
        <button style={{flex:2,backgroundColor:"#19A85A",color:"#fff",border:"none",borderRadius:16,padding:"14px",fontSize:16,fontWeight:900,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 0 #0E7A40",letterSpacing:-0.3}} onClick={function(){onRetry([]);}}>Again →</button>
      </div>
    </div>
  </div>);}
  ...
  return(
    <div style={{maxWidth:420,margin:"0 auto",display:"flex",flexDirection:"column",minHeight:"100vh",backgroundColor:"#F6F6F6",animation:"fadeIn 0.2s ease"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"8px 12px calc(32px + env(safe-area-inset-bottom, 0px))"}}>
        ...
        {visible.map(function(word){
          var srs=srsData[word.id];var sc=srs&&srs.score!=null?srs.score:-1;
            ...
            <div key={word.id} style={{backgroundColor:"#fff",borderRadius:14,marginBottom:8,border:"1px solid #E8E8E8",overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",borderLeft:"3px solid "+srsColor}}>
              <button style={{width:"100%",background:"none",border:"none",padding:"13px 14px",textAlign:"left",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:12}} onClick={function(){setExpanded(isOpen?null:word.id);}}>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,flexShrink:0}}>
                  ...
                  <span style={{fontSize:13,color:"#CCC"}}>{isOpen?"^":"v"}</span>
                </div>
              </button>
              {isOpen&&(
...
function PaywallScreen(props){
  var onSubscribe=props.onSubscribe,onContinueFree=props.onContinueFree,onStart=props.onStart,onTrial=props.onTrial,trialAvailable=props.trialAvailable,trialDays=props.trialDays||3;
  ...
  var features=[
    {label:"Full A1 plus A2, B1 and B2 levels",sub:"280+ more words across A1, A2, B1 and B2"},
  ...
  return(
    <div style={{maxWidth:420,margin:"0 auto",backgroundColor:"#F6F6F6",minHeight:"100vh",display:"flex",flexDirection:"column",animation:"fadeIn 0.2s ease"}}>
      <div style={{background:"linear-gradient(155deg,#0E7A40 0%,#19A85A 100%)",padding:"36px 24px 0",textAlign:"center",flexShrink:0,position:"relative"}}>
        ...
        <Logo size={44}/>
        <h1 style={{margin:"14px 0 6px",fontSize:28,fontWeight:900,color:"#fff",letterSpacing:-0.5}}>Ikasi Pro</h1>
        <p style={{margin:"0 0 0",fontSize:14,color:"rgba(255,255,255,0.85)",fontWeight:500}}>Unlock the full Basque vocabulary journey</p>
